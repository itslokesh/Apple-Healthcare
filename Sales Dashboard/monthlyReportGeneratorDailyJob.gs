function generateMonthlyReportBeautified() {
  ...
  if (monthlyReportRowIndex === -1) {
    ...
    configSheet.getRange(newRowIndex, 1, 1, 2).setValues([
      ["Monthly Report - New Dashboard1", 1]
    ]);
  ...
  let monthlySheet = ss.getSheetByName("Monthly Report 2025-1");
  if (!monthlySheet) monthlySheet = ss.insertSheet("Monthly Report 2025-1");
