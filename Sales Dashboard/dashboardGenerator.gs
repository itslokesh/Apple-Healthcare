function dashboardGenerator() {
  ...
  if (!sourceSheet) {
    console.error("Error: Sheet 'Sales Invoice responses' not found in the spreadsheet.");
    console.log("Available sheets: " + ss.getSheets().map(sheet => sheet.getName()).join(", "));
    throw new Error("Source sheet 'Sales Invoice responses' does not exist. Please check the sheet name.");
  }
  
  const targetSheetName = "Automation Dashboard";
  ...
  // Find the last row with data in column B (timestamp column)
  const lastRowWithData = sourceSheet.getLastRow();
  let actualDataRows = 0;
  
  // Get only column B data to check for non-empty cells
  if (lastRowWithData > 1) {
    const columnBData = sourceSheet.getRange(2, 2, lastRowWithData - 1, 1).getValues();
    actualDataRows = columnBData.filter(row => row[0] !== "" && row[0] !== null && row[0] !== undefined).length + 1; // +1 for header
  }
  
  // Read source data only up to rows that have data in column B
  const sourceData = actualDataRows > 1 ? 
    sourceSheet.getRange(1, 1, actualDataRows, sourceSheet.getLastColumn()).getValues() : 
    [sourceSheet.getRange(1, 1, 1, sourceSheet.getLastColumn()).getValues()[0]]; // Just header if no data
